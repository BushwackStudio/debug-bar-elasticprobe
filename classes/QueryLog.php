...
class QueryLog {
	...
	public function action_admin_menu() {
		add_submenu_page(
			'elasticprobe',
			esc_html__( 'Query Log', 'debug-bar-elasticprobe' ),
			esc_html__( 'Query Log', 'debug-bar-elasticprobe' ),
			'manage_options',
			'ep-query-log',
	...
	public function screen_options() {
		...
		<div class="wrap">
			...
			<form action="<?php echo esc_url( $action ); ?>" method="post">
						...
						<tr>
							...
							<td>
								...
								<span class="description">
									<?php
									echo wp_kses_post( __( 'Note that query logging can have <strong>severe</strong> performance implications on your website.', 'debug-bar-elasticprobe' ) );
									if ( $is_time_limit ) {
										echo ' ' . wp_kses_post(
											sprintf(
												/* translators: date */
												__( 'Logging queries until <strong>%s</strong>.', 'debug-bar-elasticprobe' ),
				...
				<p>
					...
					echo wp_kses_post(
						sprintf(
							...
							__( 'Please note that logs are stored until the storage limit is reached. The current limit is: <strong>%s</strong>', 'debug-bar-elasticprobe' ),
				...
				<p class="submit">
					<input type="submit" name="submit" id="submit" class="button button-primary" value="<?php esc_attr_e( 'Save Changes', 'debug-bar-elasticprobe' ); ?>">

					<?php if ( ! empty( $log ) ) : ?>
						<a class="button" href="<?php echo esc_url( add_query_arg( array( 'ep_clear_query_log' => wp_create_nonce( 'ep_clear_query_log' ) ) ) ); ?>"><?php esc_html_e( 'Empty Log', 'debug-bar-elasticprobe' ); ?></a>
					<?php endif; ?>
				</p>
			</form>
	...
	public function maybe_add_request_type( array $args, string $path, array $query_args, $type ): array {
		...
		if ( ! empty( $type ) ) {
			...
			if ( 'get' === $type ) {
				$args['ep_query_type'] = esc_html__( 'Raw ES document', 'debug-bar-elasticprobe' );
			}
		}

		if ( '_nodes/plugins' === $path ) {
			$args['ep_query_type'] = esc_html__( 'Elasticsearch check', 'debug-bar-elasticprobe' );
		}
	...
	protected function determine_request_query_type( array $request_args, string $path, string $index, string $type, array $query, array $query_args, $query_object ): string {
		if ( $query_object instanceof \WP_Query && $query_object->is_main_query() ) {
			return esc_html__( 'Main query', 'debug-bar-elasticprobe' );
		}

		if ( empty( $query['query'] ) && ! empty( $query['aggs'] ) ) {
			return esc_html__( 'Possible values for EP filter', 'debug-bar-elasticprobe' );
		}
		...
		if ( '' !== $search_term ) {
			...
			return esc_html__( 'Search', 'debug-bar-elasticprobe' );
		}
